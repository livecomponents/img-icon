...
export async function attributes(name, newValue, component) {
  switch (name) {
    case 'a11y-label':
      component.a11yLabel = newValue;
      break;
    case 'icon-set':
      await getIconConfig(newValue, component);
      break;
    case 'accessible':
      component.ariaHidden = false;
      break;
    case 'desc':
      component.desc = newValue;
      break;
    case 'fill':
...
export async function getIconConfig(path, component) {
  try {
    const defaultConfig = defaultIcons();
    if (path && path.endsWith('.json')) {
      const response = await fetch(path, JSON_OPTIONS);
      await response.json().then(({ icons, viewBox }) => {
        component.iconSet = icons;
        component.viewBox = viewBox;
      });
      return;
    }
    component.iconSet = defaultConfig.icons;
    component.viewBox = defaultConfig.viewBox;
  } catch (e) {
    console.error(e);
  }
}
