(async function runModule(window) {
  const CUSTOM_ELEMENT_NAME = 'img-icon';
  if (!window.customElements.get(CUSTOM_ELEMENT_NAME)) {
    window.customElements.define(
      CUSTOM_ELEMENT_NAME,
      class ImgIcon extends HTMLElement {
        static get observedAttributes() {
          return [
            'icons',
            'reference',
            'shape',
            'a11y-label',
            'accessible',
            'fill',
            'view',
          ];
        }

        get iconSet() {
          const icons =
            !!this.reference && !!this.reference.icons
              ? this.reference.icons
              : !!this.icons
              ? this.icons
              : [];
          return Object.keys(icons);
        }

        constructor() {
          super();
          this.root = null;
          this.hasConnected = false;
          this.accessible = true;
          this.a11yLabel = '';
          this.icons = defaultIcons();
          this.shape = 'star';
          this.fill = 100;
          this.reference = null;
          this.view = '0 0 24 24';
        }

        connectedCallback() {
          if (this.root === null) {
            this.root = this.attachShadow({ mode: 'open' });
          }
          render(this);
        }

        attributeChangedCallback(name, oldValue, newValue) {
          if (newValue === oldValue) {
            return;
          }
          attributes(name, newValue, this);
        }
      }
    );
  }
})(window);

async function attributes(name, newValue, component) {
  switch (name) {
    case 'a11y-label':
      component.a11yLabel = newValue;
      break;
    case 'reference':
      const refId = document.querySelector(`#${newValue}`);
      component.reference =
        !!refId &&
        refId.getAttribute('type') === 'application/json' &&
        !!refId.innerText &&
        isJSON(refId.innerText)
          ? JSON.parse(refId.innerText)
          : null;
    case 'icons':
      component.icons =
        !!newValue && isJSON(newValue) ? JSON.parse(newValue) : defaultIcons();
      break;
    case 'accessible':
      component.accessible = newValue === 'true';
      break;
    case 'fill':
      const parsed = parseInt(newValue, 10);
      component.fill =
        typeof parsed === 'number' && !isNaN(parsed) ? parsed : 100;
      break;
    case 'shape':
      component.shape = newValue;
      break;
    case 'view':
      component.view = newValue;
      break;
    default:
      break;
  }
  render(component);
}

function isJSON(str) {
  try {
    JSON.parse(str);
  } catch (e) {
    return false;
  }
  return true;
}

function render(component) {
  if (component.root) {
    const $template = document.createElement('template');
    $template.innerHTML = template(component);
    component.root.innerHTML = '';
    component.root.appendChild(document.importNode($template.content, true));
    component.hasConnected = true;
  }
}

function defaultIcons() {
  return {
    arrowDown: [
      'M20 12l-1.41-1.41L13 16.17V4h-2v12.17l-5.58-5.59L4 12l8 8 8-8z',
    ],
    arrowLeft: [
      'M20,11V13H8L13.5,18.5L12.08,19.92L4.16,12L12.08,4.08L13.5,5.5L8,11H20Z',
    ],
    arrowRight: [
      'M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z',
    ],
    arrowUp: ['M4 12l1.41 1.41L11 7.83V20h2V7.83l5.58 5.59L20 12l-8-8-8 8z'],
    addPhoto: [
      'M3 4V1h2v3h3v2H5v3H3V6H0V4h3zm3 6V7h3V4h7l1.83 2H21c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H5c-1.1 0-2-.9-2-2V10h3zm7 9c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm-3.2-5c0 1.77 1.43 3.2 3.2 3.2s3.2-1.43 3.2-3.2-1.43-3.2-3.2-3.2-3.2 1.43-3.2 3.2z',
    ],
    alertCircle: [
      'M13,13H11V7H13M13,17H11V15H13M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z',
    ],
    alertTriangle: ['M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z'],
    bell: [
      'M14,20A2,2 0 0,1 12,22A2,2 0 0,1 10,20H14M12,2A1,1 0 0,1 13,3V4.08C15.84,4.56 18,7.03 18,10V16L21,19H3L6,16V10C6,7.03 8.16,4.56 11,4.08V3A1,1 0 0,1 12,2Z',
    ],
    bullhorn: [
      'M12,8H4A2,2 0 0,0 2,10V14A2,2 0 0,0 4,16H5V20A1,1 0 0,0 6,21H8A1,1 0 0,0 9,20V16H12L17,20V4L12,8M21.5,12C21.5,13.71 20.54,15.26 19,16V8C20.53,8.75 21.5,10.3 21.5,12Z',
    ],
    car: [
      'M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z',
    ],
    chat: [
      'M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 9h12v2H6V9zm8 5H6v-2h8v2zm4-6H6V6h12v2z',
    ],
    chevronUp: ['M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6z'],
    chevronDown: ['M7.41 7.84L12 12.42l4.59-4.58L18 9.25l-6 6-6-6z'],
    chevronLeft: ['M15.41 16.09l-4.58-4.59 4.58-4.59L14 5.5l-6 6 6 6z'],
    chevronRight: ['M8.59 16.34l4.58-4.59-4.58-4.59L10 5.75l6 6-6 6z'],
    checkmark: ['M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z'],
    checkmarkCircle: [
      'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z',
    ],
    checkmarkCircleOutline: [
      'M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2zm0 2a8 8 0 1 0 0 16 8 8 0 0 0 0-16zm-1 12.502l-4.5-4.5 1.414-1.414L11 13.674l5.586-5.586L18 9.502l-7 7z',
    ],
    clipboard: [
      'M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm2 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z',
    ],
    clock: [
      'M12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22C6.47,22 2,17.5 2,12A10,10 0 0,1 12,2M12.5,7V12.25L17,14.92L16.25,16.15L11,13V7H12.5Z',
    ],
    close: [
      'M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z',
    ],
    cloudUp: [
      'M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z',
    ],
    creditCard: [
      'M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z',
    ],
    dashboard: [
      'M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z',
    ],
    description: [
      'M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z',
    ],
    directions: [
      'M21.71 11.29l-9-9c-.39-.39-1.02-.39-1.41 0l-9 9c-.39.39-.39 1.02 0 1.41l9 9c.39.39 1.02.39 1.41 0l9-9c.39-.38.39-1.01 0-1.41zM14 14.5V12h-4v3H8v-4c0-.55.45-1 1-1h5V7.5l3.5 3.5-3.5 3.5Z',
    ],
    dotsVertical: [
      'M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z',
    ],
    download: ['M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z'],
    earth: [
      'M17.9,17.39C17.64,16.59 16.89,16 16,16H15V13A1,1 0 0,0 14,12H8V10H10A1,1 0 0,0 11,9V7H13A2,2 0 0,0 15,5V4.59C17.93,5.77 20,8.64 20,12C20,14.08 19.2,15.97 17.9,17.39M11,19.93C7.05,19.44 4,16.08 4,12C4,11.38 4.08,10.78 4.21,10.21L9,15V16A2,2 0 0,0 11,18M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z',
    ],
    email: [
      'M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z',
    ],
    emailOutline: [
      'M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 14H4V8l8 5 8-5v10zm-8-7L4 6h16l-8 5z',
    ],
    fileCSV: [
      'M 6 2 C 4.898438 2 4 2.898438 4 4 L 4 8 C 2.894531 8 2 8.894531 2 10 L 2 15 C 2 16.105469 2.894531 17 4 17 L 4 20 C 4 21.101563 4.898438 22 6 22 L 18 22 C 19.101563 22 20 21.101563 20 20 L 20 4 C 20 2.898438 19.101563 2 18 2 Z M 6 4 L 18 4 L 18 10 C 18 8.894531 17.105469 8 16 8 L 6 8 Z M 5.71875 10.03125 C 7.203125 10.03125 7.367188 11.199219 7.40625 11.75 L 6.40625 11.75 C 6.40625 11.34375 6.382813 10.84375 5.71875 10.84375 C 5.457031 10.84375 4.96875 10.8125 4.96875 12.09375 L 4.96875 12.9375 C 4.96875 13.863281 5.148438 14.1875 5.6875 14.1875 C 6.257813 14.1875 6.40625 13.832031 6.40625 13.3125 L 7.40625 13.3125 C 7.382813 13.867188 7.167969 15 5.6875 15 C 3.988281 15 3.96875 13.273438 3.96875 12.9375 L 3.96875 12.09375 C 3.96875 11.246094 4.28125 10.03125 5.71875 10.03125 Z M 9.8125 10.03125 C 11.269531 10.03125 11.34375 11.3125 11.34375 11.53125 L 10.34375 11.53125 C 10.34375 11.429688 10.3125 10.8125 9.78125 10.8125 C 9.25 10.8125 9.21875 11.296875 9.21875 11.375 C 9.214844 12.21875 11.34375 12 11.34375 13.65625 C 11.34375 13.871094 11.28125 15 9.8125 15 C 8.207031 15 8.125 13.679688 8.125 13.40625 L 9.125 13.40625 C 9.125 13.550781 9.054688 14.21875 9.8125 14.21875 C 9.90625 14.21875 10.34375 14.167969 10.34375 13.65625 C 10.34375 12.585938 8.25 13.03125 8.25 11.375 C 8.25 11.175781 8.414063 10.03125 9.8125 10.03125 Z M 12.03125 10.09375 L 13.125 10.09375 L 13.9375 13.59375 L 14.78125 10.09375 L 15.875 10.09375 L 14.46875 14.9375 L 13.4375 14.9375 Z M 18 15 L 18 20 L 6 20 L 6 17 L 16 17 C 17.105469 17 18 16.105469 18 15 Z ',
    ],
    listCurled: [
      'M4.10180624,3 L4.10180624,14.5599343 L6.20361248,14.5599343 L6.20361248,5.10180624 L19.865353,5.10180624 L19.865353,18.7635468 C19.865353,19.3573386 19.4082417,19.8144499 18.8144499,19.8144499 C18.2343514,19.8144499 17.7635468,19.3425944 17.7635468,18.7635468 L17.7635468,16.6617406 L2,16.6617406 L2,18.7635468 C2,20.5017406 3.4145156,21.9162562 5.15270936,21.9162562 L18.8144499,21.9162562 C20.543154,21.9162562 21.9671593,20.4922509 21.9671593,18.7635468 L21.9671593,3 L4.10180624,3 Z M8.30541872,8.2545156 L8.30541872,10.3563218 L10.407225,10.3563218 L10.407225,8.2545156 L8.30541872,8.2545156 Z M12.5090312,8.2545156 L12.5090312,10.3563218 L17.7635468,10.3563218 L17.7635468,8.2545156 L12.5090312,8.2545156 Z M8.30541872,12.4581281 L8.30541872,14.5599343 L10.407225,14.5599343 L10.407225,12.4581281 L8.30541872,12.4581281 Z M12.5090312,12.4581281 L12.5090312,14.5599343 L17.7635468,14.5599343 L17.7635468,12.4581281 L12.5090312,12.4581281 Z',
    ],
    filter: ['M10 18h4v-2h-4v2zM3 6v2h18V6H3zm3 7h12v-2H6v2z'],
    flag: ['M14.4 6L14 4H5v17h2v-7h5.6l.4 2h7V6z'],
    fullscreen: [
      'M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z',
    ],
    fullscreenExit: [
      'M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z',
    ],
    heart: [
      'M12,21.35L10.55,20.03C5.4,15.36 2,12.27 2,8.5C2,5.41 4.42,3 7.5,3C9.24,3 10.91,3.81 12,5.08C13.09,3.81 14.76,3 16.5,3C19.58,3 22,5.41 22,8.5C22,12.27 18.6,15.36 13.45,20.03L12,21.35Z',
    ],
    heartOutline: [
      'M12.1,18.55L12,18.65L11.89,18.55C7.14,14.24 4,11.39 4,8.5C4,6.5 5.5,5 7.5,5C9.04,5 10.54,6 11.07,7.36H12.93C13.46,6 14.96,5 16.5,5C18.5,5 20,6.5 20,8.5C20,11.39 16.86,14.24 12.1,18.55M16.5,3C14.76,3 13.09,3.81 12,5.08C10.91,3.81 9.24,3 7.5,3C4.42,3 2,5.41 2,8.5C2,12.27 5.4,15.36 10.55,20.03L12,21.35L13.45,20.03C18.6,15.36 22,12.27 22,8.5C22,5.41 19.58,3 16.5,3Z',
    ],
    helpCircle: [
      'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17h-2v-2h2v2zm2.07-7.75l-.9.92C13.45 12.9 13 13.5 13 15h-2v-.5c0-1.1.45-2.1 1.17-2.83l1.24-1.26c.37-.36.59-.86.59-1.41 0-1.1-.9-2-2-2s-2 .9-2 2H8c0-2.21 1.79-4 4-4s4 1.79 4 4c0 .88-.36 1.68-.93 2.25z',
    ],
    helpCircleOutline: [
      'M11 18h2v-2h-2v2zm1-16C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm0-14c-2.21 0-4 1.79-4 4h2c0-1.1.9-2 2-2s2 .9 2 2c0 2-3 1.75-3 5h2c0-2.25 3-2.5 3-5 0-2.21-1.79-4-4-4z',
    ],
    home: ['M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z'],
    infoCircle: [
      'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z',
    ],
    infoCircleOutline: [
      'M11 17h2v-6h-2v6zm1-15C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zM11 9h2V7h-2v2z',
    ],
    lock: [
      'M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z',
    ],
    location: [
      'M12,11.5A2.5,2.5 0 0,1 9.5,9A2.5,2.5 0 0,1 12,6.5A2.5,2.5 0 0,1 14.5,9A2.5,2.5 0 0,1 12,11.5M12,2A7,7 0 0,0 5,9C5,14.25 12,22 12,22C12,22 19,14.25 19,9A7,7 0 0,0 12,2Z',
    ],
    menu: ['M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z'],
    mobilePhone: [
      'M15.5 1h-8C6.12 1 5 2.12 5 3.5v17C5 21.88 6.12 23 7.5 23h8c1.38 0 2.5-1.12 2.5-2.5v-17C18 2.12 16.88 1 15.5 1zm-4 21c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm4.5-4H7V4h9v14z',
    ],
    money: [
      'M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z',
    ],
    minus: ['M19 13H5v-2h14v2z'],
    pencil: [
      'M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z',
    ],
    plus: ['M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z'],
    phone: [
      'M6.62,10.79C8.06,13.62 10.38,15.94 13.21,17.38L15.41,15.18C15.69,14.9 16.08,14.82 16.43,14.93C17.55,15.3 18.75,15.5 20,15.5A1,1 0 0,1 21,16.5V20A1,1 0 0,1 20,21A17,17 0 0,1 3,4A1,1 0 0,1 4,3H7.5A1,1 0 0,1 8.5,4C8.5,5.25 8.7,6.45 9.07,7.57C9.18,7.92 9.1,8.31 8.82,8.59L6.62,10.79Z',
    ],
    photoCollection: [
      'M22 16V4c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2zm-11-4l2.03 2.71L16 11l4 5H8l3-4zM2 6v14c0 1.1.9 2 2 2h14v-2H4V6H2z',
    ],
    print: [
      'M19 8H5c-1.66 0-3 1.34-3 3v6h4v4h12v-4h4v-6c0-1.66-1.34-3-3-3zm-3 11H8v-5h8v5zm3-7c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm-1-9H6v4h12V3z',
    ],
    reply: ['M10 9V5l-7 7 7 7v-4.1c5 0 8.5 1.6 11 5.1-1-5-4-10-11-11z'],
    replyAll: [
      'M7 8V5l-7 7 7 7v-3l-4-4 4-4zm6 1V5l-7 7 7 7v-4.1c5 0 8.5 1.6 11 5.1-1-5-4-10-11-11z',
    ],
    rotateLeft: [
      'M7.11 8.53L5.7 7.11C4.8 8.27 4.24 9.61 4.07 11h2.02c.14-.87.49-1.72 1.02-2.47zM6.09 13H4.07c.17 1.39.72 2.73 1.62 3.89l1.41-1.42c-.52-.75-.87-1.59-1.01-2.47zm1.01 5.32c1.16.9 2.51 1.44 3.9 1.61V17.9c-.87-.15-1.71-.49-2.46-1.03L7.1 18.32zM13 4.07V1L8.45 5.55 13 10V6.09c2.84.48 5 2.94 5 5.91s-2.16 5.43-5 5.91v2.02c3.95-.49 7-3.85 7-7.93s-3.05-7.44-7-7.93z',
    ],
    rotateRight: [
      'M15.55 5.55L11 1v3.07C7.06 4.56 4 7.92 4 12s3.05 7.44 7 7.93v-2.02c-2.84-.48-5-2.94-5-5.91s2.16-5.43 5-5.91V10l4.55-4.45zM19.93 11c-.17-1.39-.72-2.73-1.62-3.89l-1.42 1.42c.54.75.88 1.6 1.02 2.47h2.02zM13 17.9v2.02c1.39-.17 2.74-.71 3.9-1.61l-1.44-1.44c-.75.54-1.59.89-2.46 1.03zm3.89-2.42l1.42 1.41c.9-1.16 1.45-2.5 1.62-3.89h-2.02c-.14.87-.48 1.72-1.02 2.48z',
    ],
    save: [
      'M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z',
    ],
    search: [
      'M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z',
    ],
    star: [
      'M12,17.27L18.18,21L16.54,13.97L22,9.24L14.81,8.62L12,2L9.19,8.62L2,9.24L7.45,13.97L5.82,21L12,17.27Z',
    ],
    starHalf: [
      'M12,15.4V6.1L13.71,10.13L18.09,10.5L14.77,13.39L15.76,17.67M22,9.24L14.81,8.63L12,2L9.19,8.63L2,9.24L7.45,13.97L5.82,21L12,17.27L18.18,21L16.54,13.97L22,9.24Z',
    ],
    starOutline: [
      'M12,15.39L8.24,17.66L9.23,13.38L5.91,10.5L10.29,10.13L12,6.09L13.71,10.13L18.09,10.5L14.77,13.38L15.76,17.66M22,9.24L14.81,8.63L12,2L9.19,8.63L2,9.24L7.45,13.97L5.82,21L12,17.27L18.18,21L16.54,13.97L22,9.24Z',
    ],
    shieldCheckmark: [
      'M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-2 16l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8Z',
    ],
    tools: [
      'M 5 3 L 3 5 L 4.40625 7.109375 C 4.77725 7.666375 5.4023125 8 6.0703125 8 L 6.5859375 8 L 9.2929688 10.707031 L 10.707031 9.2929688 L 8 6.5859375 L 8 6.0703125 C 8 5.4023125 7.666375 4.77725 7.109375 4.40625 L 5 3 z M 18 3 C 15.791 3 14 4.791 14 7 C 14 7.5862513 14.132749 8.1389203 14.359375 8.640625 L 4.2929688 18.707031 C 3.9019687 19.098031 3.9019687 19.731094 4.2929688 20.121094 L 4.8789062 20.707031 C 5.2689063 21.098031 5.9019687 21.098031 6.2929688 20.707031 L 12.128906 14.871094 L 17.855469 20.597656 C 18.391469 21.133656 19.258922 21.134656 19.794922 20.597656 L 20.597656 19.794922 C 21.133656 19.259922 21.133656 18.391469 20.597656 17.855469 L 14.871094 12.128906 L 16.359375 10.640625 C 16.86108 10.867251 17.413749 11 18 11 C 20.209 11 22 9.209 22 7 C 22 6.414 21.867625 5.860375 21.640625 5.359375 L 19 8 L 17 6 L 19.640625 3.359375 C 19.139625 3.132375 18.586 3 18 3 z',
    ],
    trash: [
      'M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z',
    ],
    triangleArrowDown: ['M7 10l5 5 5-5z'],
    triangleArrowUp: ['M7 14l5-5 5 5z'],
    upload: ['M9,16V10H5L12,3L19,10H15V16H9M5,20V18H19V20H5Z'],
    userCircle: [
      'M12,2 C6.48,2 2,6.48 2,12 C2,17.52 6.48,22 12,22 C17.52,22 22,17.52 22,12 C22,6.48 17.52,2 12,2 Z M12,5 C13.66,5 15,6.34 15,8 C15,9.66 13.66,11 12,11 C10.34,11 9,9.66 9,8 C9,6.34 10.34,5 12,5 Z M12,19.2 C9.5,19.2 7.29,17.92 6,15.98 C6.03,13.99 10,12.9 12,12.9 C13.99,12.9 17.97,13.99 18,15.98 C16.71,17.92 14.5,19.2 12,19.2 Z',
    ],
    visibilityHide: [
      'M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.83l2.92 2.92c1.51-1.26 2.7-2.89 3.43-4.75-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C10.74 7.13 11.35 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.31-.78l3.15 3.15.02-.16c0-1.66-1.34-3-3-3l-.17.01z',
    ],
    visibilityShow: [
      'M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z',
    ],
    wrench: [
      'M22.7,19L13.6,9.9C14.5,7.6 14,4.9 12.1,3C10.1,1 7.1,0.6 4.7,1.7L9,6L6,9L1.6,4.7C0.4,7.1 0.9,10.1 2.9,12.1C4.8,14 7.5,14.5 9.8,13.6L18.9,22.7C19.3,23.1 19.9,23.1 20.3,22.7L22.6,20.4C23.1,20 23.1,19.3 22.7,19Z',
    ],
    zoomIn: [
      'M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14zm2.5-4h-2v2H9v-2H7V9h2V7h1v2h2v1z',
    ],
    zoomOut: [
      'M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14zM7 9h5v1H7z',
    ],
  };
}

function template(component) {
  if (!component) {
    return '';
  }
  const { accessible, a11yLabel, fill, icons, reference, shape, view } =
    component;
  console.log({ a11yLabel, accessible });
  const BASE_ICON_COLOR = '#e0e0e0';
  const ICON_COLOR = '#fff';
  const fillType =
    fill > 0 && fill < 100
      ? 'gradient'
      : fill >= 100
      ? 'foreground-color'
      : 'background-color';
  return /* html */ `
    <style>
      :host {
        display: inline-block;
        margin: 0;
        padding: 0;
        width: var(--img-icon--maxsize, 1rem);
        min-width: var(--img-icon--maxsize, 1rem);
        height: var(--img-icon--maxsize, 1rem);
        min-height: var(--img-icon--maxsize, 1rem);
        max-width: var(--img-icon--maxsize, 1rem);
        max-height: var(--img-icon--maxsize, 1rem);
        outline-color: var(--img-icon--outline-color, rgba(255, 255, 255, 0));
      }

      :host(:focus),
      :host(:hover),
      :host(:active) {
        --img-icon--base-color: var(--img-icon--base-color-action, currentColor);
        --img-icon--color: var(--img-icon--color-action, currentColor);
      }

      .icon {
        display: block;
        width: var(--img-icon--maxsize, 1rem);
        min-width: var(--img-icon--maxsize, 1rem);
        height: var(--img-icon--maxsize, 1rem);
        min-height: var(--img-icon--maxsize, 1rem);
        max-width: var(--img-icon--maxsize, 1rem);
        max-height: var(--img-icon--maxsize, 1rem);
        stroke: none;
      }
      .background-stop-color {
        stop-color: var(--img-icon--base-color, ${BASE_ICON_COLOR});
      }
      .background-color {
        fill: var(--img-icon--base-color, ${BASE_ICON_COLOR});
      }
      .foreground-color {
        fill: var(--img-icon--color, ${ICON_COLOR});
      }
      .foreground-stop-color {
        stop-color: var(--img-icon--color, ${ICON_COLOR});
      }
      .gradient {
        fill: url(#gradient);
      }
    </style>
    <svg
      role="img"
      preserveAspectRatio="xMidYMid meet"
      class="icon ${fillType}"
      ${!accessible ? 'aria-hidden="true"' : ''}
      ${accessible && !!a11yLabel ? `aria-label="${a11yLabel}"` : ''}
      viewBox="${!!reference && !!reference.view ? reference.view : view}"
    >
      <defs>
        <linearGradient id="gradient">
          <stop class="foreground-stop-color" offset="${fill}%" />
          <stop class="background-stop-color" offset="${100 - fill}%" />
        </linearGradient>
      </defs>
      ${(!!reference && !!reference.icons && !!reference.icons[shape]
        ? reference.icons[shape]
        : !!icons && !!icons[shape]
        ? icons[shape]
        : ''
      )
        .map((path) => `<path d="${path}" />`)
        .join('\n')}
    </svg>
  `;
}
